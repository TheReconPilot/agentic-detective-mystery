...
- **M7 — Polish** ✅ Done. README expanded with: a Mermaid architecture diagram that includes the M9 commitment loop and the M10 confrontation surface; a sample session generated against the real `evals/cases/42.json` fixture showing voice + commitments + `show`; badges for CI, license, Python version, ruff, mypy, uv; an updated tests-and-coverage table (173 tests, 94% line coverage). GitHub Actions CI runs ruff format/check, mypy strict, and pytest on every push and PR ([.github/workflows/ci.yml](.github/workflows/ci.yml)); coverage is uploaded as an artifact. Tagged `v0.1.0`. The asciinema recording from the original M7 plan was replaced with the in-repo text sample on user feedback — text is reviewable in PR diffs and survives terminal-emulator drift.
